...
@section('content')
    <div class="container mx-auto px-4">
        <div class="gameDetails border-b border-gray-800 pb-12 flex flex-col lg:flex-row">
            <div class="flex-none">
                <img src="{{ $game['coverImageUrl'] }}" alt="{{ $game['name'] }}" title="{{ $game['name'] }}">
            </div>
            ...
            <div class="lg:ml-12 lg:mr-64">
                ...
                <div class="text-gray-400 mt-3">
                    ...
                    <span>{{ $game['involvedCompanies'] }}</span> &middot;
                    <span>{{ $game['platformAbbreviations'] }}</span>
                </div>
                    ...
                    <div class="flex items-center mt-4 sm:mt-0 sm:ml-12 space-x-4">
                        @if ($game['social']['website'])
                            <div class="w-8 h-8 flex items-center justify-center bg-gray-800 rounded-full">
                                <a href="{{ $game['social']['website']['url'] }}" class="hover:text-indigo-500 focus:text-indigo-500" target="_blank">
                        ...
                        @if ($game['social']['facebook'])
                            <div class="w-8 h-8 flex items-center justify-center bg-gray-800 rounded-full">
                                <a href="{{ $game['social']['facebook']['url'] }}" class="hover:text-indigo-500 focus:text-indigo-500" target="_blank">
                        ...
                        <div class="w-8 h-8 flex items-center justify-center bg-gray-800 rounded-full">
                            <a href="{{ $game['social']['twitter']['url'] }}" class="hover:text-indigo-500 focus:text-indigo-500" target="_blank">
                        ...
                        <div class="w-8 h-8 flex items-center justify-center bg-gray-800 rounded-full">
                            <a href="{{ $game['social']['instagram']['url'] }}" class="hover:text-indigo-500 focus:text-indigo-500" target="_blank">
                ...
                <div class="mt-12">
                    ...
                    <a href="{{ $game['trailerVideo'] }}" class="inline-flex bg-blue-500 text-white font-semibold px-4 py-4 hover:bg-blue-600 rounded transition ease-in-out duration-150">
                        <svg class="w-6 fill-current" viewBox="0 0 24 24"><path d="M0 0h24v24H0z" fill="none"></path><path d="M10 16.5l6-4.5-6-4.5v9zM12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8z"></path></svg>
                        <span class="ml-2">Play Trailer</span>
                    </a>
                </div>
        ...
        <div class="imagesContainer border-b border-gray-800 pb-12 mt-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 mt-8">
                @foreach ($game['screenshots'] as $screenshot)
                    <div>
                        <a href="{{ $screenshot['huge'] }}" target="_blank">
                            <img src="{{ $screenshot['big'] }}" alt="{{ $game['name'] }} - Screenshot #{{ ++$loop->index }}" class="transition ease-in-out duration-150 hover:opacity-75" />
                        </a>
                    </div>
                @endforeach
            </div>
                    ...
                    <div class="game mt-8">
                        <div class="relative inline-block">
                            <a href="{{ route('games.show', $similarGame['slug']) }}">
                                <img src="{{ $similarGame['coverImageUrl'] }}" alt="{{ $similarGame['name'] }}" class="hover:opacity-75 transition ease-in-out duration-150">
                            </a>

                            <div class="absolute bottom-0 right-0 w-16 h-16 bg-gray-800 rounded-full" style="bottom: -1.4rem; right: -1.4rem;">
                                <div class="h-full flex items-center justify-center font-bold text-xs">{{ $similarGame['rating'] }}</div>
                            </div>
                        </div>
                        ...
                        <div class="text-gray-400 mt-1">{{ $similarGame['platformAbbreviations'] }}</div>
                    </div>
                @endforeach
            </div>
        </div>
